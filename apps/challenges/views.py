...
@swagger_auto_schema(
    ...
    responses={
        ...
        status.HTTP_403_FORBIDDEN: openapi.Response("{'error': 'You are not authorized to make this request'}"),
    },
)
@api_view(["GET"])
@throttle_classes([UserRateThrottle])
...
def get_participant_teams_for_challenge(request, challenge_pk):
    ...
    if is_user_a_host_of_challenge(request.user, challenge_pk):
        ...
        serializer = ParticipantTeamDetailSerializer(participant_teams, many=True)
...
def add_participant_team_to_challenge(request, challenge_pk, participant_team_pk):
    ...
    participant_team_user_ids = set(Participant.objects.select_related("user").filter(team__id=participant_team_pk).values_list("user", flat=True))

    for user in participant_team_user_ids:
        if has_user_participated_in_challenge(user, challenge_pk):
            response_data = {
                "error": "Sorry, other team member(s) have already participated in the Challenge. Please participate with a different team!",
                "challenge_id": int(challenge_pk),
                "participant_team_id": int(participant_team_pk),
            }
            return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)

    if not (is_user_part_of_participant_team(request.user, participant_team) or is_user_creator_of_participant_team(request.user, participant_team)):
        response_data = {"error": "Sorry, you are not authorized to to make this request"}
        return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
...
def challenge_phase_list(request, challenge_pk):
    ...
    if request.method == "GET":
        if is_user_a_host_of_challenge(request.user, challenge_pk):
            challenge_phase = ChallengePhase.objects.filter(challenge=challenge).order_by("pk")
        else:
            challenge_phase = ChallengePhase.objects.filter(challenge=challenge, is_public=True).order_by("pk")
        ...
        serializer = ChallengePhaseCreateSerializer(data=request.data, context={"challenge": challenge})
        if serializer.is_valid():
            serializer.save()
...
def challenge_phase_detail(request, challenge_pk, pk):
    ...
    if request.method == "GET":
        if not is_user_a_host_of_challenge(request.user, challenge.id):
            ...
            serializer = ChallengePhaseCreateSerializer(challenge_phase, context={"request": request})
            response_data = serializer.data
            return Response(response_data, status=status.HTTP_200_OK)
...
def challenge_phase_split_list(request, challenge_pk):
    ...
    challenge_phase_split = ChallengePhaseSplit.objects.filter(challenge_phase__challenge=challenge).order_by("pk")
...
def create_challenge_using_zip_file(request, challenge_host_team_pk):
    ...
    if is_challenge_template:
        ...
        try:
            challenge_template = ChallengeTemplate.objects.get(id=template_id, is_active=True)
        except ChallengeTemplate.DoesNotExist:
            response_data = {"error": "Sorry, a server error occured while creating the challenge. Please try again!"}
            return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
        ...
        challenge_template_download_location = join(BASE_LOCATION, "{}.zip".format(unique_folder_name))

        try:
            response = requests.get(template_zip_s3_url, stream=True)
        except Exception as e:
            logger.error("Failed to fetch file from {}, error {}".format(template_zip_s3_url, e))
            response_data = {"error": "Sorry, there was an error in the server"}
        ...
        challenge_zip_file = SimpleUploadedFile(zip_file.name, zip_file.read(), content_type="application/zip")
        ...
        serializer = ChallengeConfigSerializer(data=challenge_data_from_hosts, context={"request": request})
    else:
        data = request.data.copy()
    ...
    for name in zip_ref.namelist():
        if (name.endswith(".yaml") or name.endswith(".yml")) and (not name.startswith("__MACOSX")):  # Ignore YAML File in __MACOSX Directory
            yaml_file = name
    ...
    try:
        ...
        message = "{} in line {}, column {}".format(error_description, line_number, column_number)
        response_data = {"error": message}
        return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
    ...
    try:
        ...
        message = "There is no key for evaluation script in YAML file. Please add it and then try again!"
        response_data = {"error": message}
        return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
    ...
    if isfile(evaluation_script_path):
        ...
        message = "No evaluation script is present in the zip file. Please add it and then try again!"
        response_data = {"error": message}
        return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
    ...
    try:
        ...
        message = "No challenge phase key found. Please add challenge phases in YAML file and try again!"
        response_data = {"error": message}
        return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
    ...
    for data in challenge_phases_data:
        ...
        if test_annotation_file:
            ...
            if not isfile(test_annotation_file_path):
                message = "No test annotation file found in zip file for challenge phase '{}'. Please add it and  then try again!".format(data["name"])
                response_data = {"error": message}
                return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
        else:
            message = "There is no key for test annotation file for challenge phase {} in yaml file. Please add it and then try again!".format(data["name"])
            response_data = {"error": message}
            return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
        ...
        if data.get("submission_meta_attributes"):
            for attribute in data["submission_meta_attributes"]:
                ...
                if len(missing_keys) == 0:
                    ...
                    message = "Please enter the following to the submission meta attribute in phase {}: {}.".format(data["id"], missing_keys_string)
                    response_data = {"error": message}
                    return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
            ...
            for attribute in data["default_submission_meta_attributes"]:
                ...
                if len(missing_keys) == 0:
                    ...
                    if not attribute["name"] in valid_attributes:
                        message = "Default meta attribute: {} in phase: {} does not exist!".format(attribute["name"], data["id"])
                        response_data = {"error": message}
                        return Response(
                            response_data,
                            status=status.HTTP_406_NOT_ACCEPTABLE,
                    ...
                    missing_keys_string = ", ".join(missing_keys)
                    message = "Please enter the following to the default submission meta attribute in phase {}: {}.".format(data["id"], missing_keys_string)
                    response_data = {"error": message}
                    return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
    ...
    if image and (image.endswith(".jpg") or image.endswith(".jpeg") or image.endswith(".png")):
        challenge_image_path = join(BASE_LOCATION, unique_folder_name, extracted_folder_name, image)
    ...
    try:
        ...
        message = "There is no key for description. Please add it and then try again!"
        response_data = {"error": message}
    ...
    try:
        ...
        if challenge_evaluation_details_file_path.endswith(".html") and isfile(challenge_evaluation_details_file_path):
            yaml_file_data["evaluation_details"] = get_file_content(challenge_evaluation_details_file_path, "rb").decode("utf-8")
        else:
            yaml_file_data["evaluation_details"] = None
    except KeyError:
        message = "There is no key for evalutaion details. Please add it and then try again!"
        response_data = {"error": message}
        return Response(response_data, status.HTTP_406_NOT_ACCEPTABLE)
    ...
    try:
        ...
        if challenge_terms_and_cond_file_path.endswith(".html") and isfile(challenge_terms_and_cond_file_path):
            yaml_file_data["terms_and_conditions"] = get_file_content(challenge_terms_and_cond_file_path, "rb").decode("utf-8")
        else:
            yaml_file_data["terms_and_conditions"] = None
    except KeyError:
        message = "There is no key for terms and conditions. Please add it and then try again!"
        response_data = {"error": message}
        return Response(response_data, status.HTTP_406_NOT_ACCEPTABLE)
    ...
    try:
        ...
        if submission_guidelines_file_path.endswith(".html") and isfile(submission_guidelines_file_path):
            yaml_file_data["submission_guidelines"] = get_file_content(submission_guidelines_file_path, "rb").decode("utf-8")
        else:
            yaml_file_data["submission_guidelines"] = None
    except KeyError:
        message = "There is no key for submission guidelines. Please add it and then try again!"
        response_data = {"error": message}
        return Response(response_data, status.HTTP_406_NOT_ACCEPTABLE)
    ...
    if leaderboard_schema:
        if "schema" not in leaderboard_schema[0]:
            message = "There is no leaderboard schema in the YAML configuration file. Please add it and then try again!"
            response_data = {"error": message}
            return Response(response_data, status.HTTP_406_NOT_ACCEPTABLE)
        if "default_order_by" not in leaderboard_schema[0].get("schema"):
            message = "There is no 'default_order_by' key in leaderboard schema. Please add it and then try again!"
            response_data = {"error": message}
            return Response(response_data, status.HTTP_406_NOT_ACCEPTABLE)
        if "labels" not in leaderboard_schema[0].get("schema"):
            message = "There is no 'labels' key in leaderboard schema. Please add it and then try again!"
            response_data = {"error": message}
            return Response(response_data, status.HTTP_406_NOT_ACCEPTABLE)
    else:
        message = "There is no key 'leaderboard' in the YAML file. Please add it and then try again!"
        response_data = {"error": message}
        return Response(response_data, status.HTTP_406_NOT_ACCEPTABLE)
    ...
    if is_challenge_template:
        ...
        challenge_phases_from_hosts = challenge_data_from_hosts.get("challenge_phases")
        ...
        for challenge_phase_data, challenge_phase_data_from_hosts in zip(challenge_phases, challenge_phases_from_hosts):
            for field in challenge_phase_fields:
                challenge_phase_data[field] = challenge_phase_data_from_hosts.get(field)
    try:
        with transaction.atomic():
            ...
            for data in yaml_file_data_of_leaderboard:
                serializer = LeaderboardSerializer(data=data, context={"config_id": data["id"]})
                if serializer.is_valid():
                    serializer.save()
            ...
            for data in challenge_phases_data:
                ...
                if phase_description_file_path.endswith(".html") and isfile(phase_description_file_path):
                    data["description"] = get_file_content(phase_description_file_path, "rb").decode("utf-8")
                else:
            ...
            for data in yaml_file_data_of_dataset_split:
                serializer = DatasetSplitSerializer(data=data, context={"config_id": data["id"]})
                if serializer.is_valid():
                    serializer.save()
            ...
            try:
                ...
                message = "There is no key for challenge phase splits. Please add it and then try again!"
                response_data = {"error": message}
                return Response(response_data, status.HTTP_406_NOT_ACCEPTABLE)

            for data in challenge_phase_splits_data:
                if challenge_phase_ids.get(str(data["challenge_phase_id"])) is None:
                    message = "Challenge phase with phase id {} doesn't exist.".format(data["challenge_phase_id"])
                    response_data = {"error": message}
                    return Response(response_data, status.HTTP_406_NOT_ACCEPTABLE)
                if leaderboard_ids.get(str(data["leaderboard_id"])) is None:
                    message = "Leaderboard with id {} doesn't exist.".format(data["leaderboard_id"])
                    response_data = {"error": message}
                    return Response(response_data, status.HTTP_406_NOT_ACCEPTABLE)
                leaderboard = leaderboard_ids[str(data["leaderboard_id"])]
                if dataset_split_ids.get(str(data["dataset_split_id"])) is None:
                    message = "Dataset split with id {} doesn't exist.".format(data["dataset_split_id"])
                    response_data = {"error": message}
                    return Response(response_data, status.HTTP_406_NOT_ACCEPTABLE)
        ...
        if zip_config:
            ...
            if challenge.is_docker_based:
                challenge_evaluation_cluster = ChallengeEvaluationCluster(challenge=zip_config.challenge)
                evaluation_cluster_serializer = ChallengeEvaluationClusterSerializer(
                    challenge_evaluation_cluster,
                    data={"name": "{0}-cluster".format(challenge.title.replace(" ", "-"))},
                    partial=True,
                )
            ...
            if not challenge.is_docker_based and challenge.inform_hosts and not challenge.remote_evaluation:
                try:
                    ...
                    if count:
                        ...
                        template_id = settings.SENDGRID_SETTINGS.get("TEMPLATES").get("WORKER_START_EMAIL")
                        send_emails(emails, template_id, template_data)
                except Exception:
                    logger.exception("Failed to start workers for challenge {}".format(zip_config.challenge.pk))

            response_data = {"success": "Challenge {} has been created successfully and sent for review to EvalAI Admin.".format(challenge.title)}
            return Response(response_data, status=status.HTTP_201_CREATED)
        ...
        try:
            ...
            try:
                ...
                logger.exception("Zip folder for challenge {} is not removed from {} location".format(challenge.pk, BASE_LOCATION))
...
def get_all_submissions_of_challenge(request, challenge_pk, challenge_phase_pk):
    ...
    if is_user_a_host_of_challenge(user=request.user, challenge_pk=challenge_pk):
        ...
        submissions = Submission.objects.filter(challenge_phase=challenge_phase, ignore_submission=False).order_by("-submitted_at")
...
@swagger_auto_schema(
    ...
    responses={
        status.HTTP_200_OK: openapi.Response(""),
        status.HTTP_400_BAD_REQUEST: openapi.Response("{'error': 'The file type requested is not valid!'}"),
        status.HTTP_404_NOT_FOUND: openapi.Response("{'error': 'Challenge Phase does not exist'}"),
    },
)
@swagger_auto_schema(
    methods=["post"],
    ...
    responses={
        status.HTTP_200_OK: openapi.Response(""),
        status.HTTP_400_BAD_REQUEST: openapi.Response("{'error': 'The file type requested is not valid!'}"),
        status.HTTP_401_UNAUTHORIZED: openapi.Response("{'error': 'Sorry, you do not belong to this Host Team!'}"),
    },
)
@api_view(["GET", "POST"])
@throttle_classes([UserRateThrottle])
        ...
        if file_type == "csv":
            if is_user_a_host_of_challenge(user=request.user, challenge_pk=challenge_pk):
                ...
                for submission in submissions.data:
                    submission_meta_attributes = parse_submission_meta_attributes(submission)
                    writer.writerow(
                        [
                            submission["id"],
                            ...
                            ",".join(affiliation for affiliation in submission["participant_team_members_affiliations"]),
                ...
                for submission in submissions.data:
                    ...
                    for field in request.data:
                        if field == "participant_team_members":
                            row.append(",".join(username["username"] for username in submission["participant_team_members"]))
                        elif field == "participant_team_members_email":
                            row.append(",".join(email["email"] for email in submission["participant_team_members"]))
                        elif field == "participant_team_members_affiliation":
                            row.append(",".join(affiliation for affiliation in submission["participant_team_members_affiliations"]))
...
def get_or_update_leaderboard(request, leaderboard_pk):
    ...
    if request.method == "PATCH":
        ...
        serializer = LeaderboardSerializer(leaderboard, data=request.data, partial=True)
...
def get_or_update_dataset_split(request, dataset_split_pk):
    ...
    if request.method == "PATCH":
        serializer = DatasetSplitSerializer(dataset_split, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            response_data = serializer.data
            return Response(response_data, status=status.HTTP_200_OK)
...
def get_or_update_challenge_phase_split(request, challenge_phase_split_pk):
    ...
    if request.method == "PATCH":
        serializer = ZipChallengePhaseSplitSerializer(challenge_phase_split, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            response_data = serializer.data
            return Response(response_data, status=status.HTTP_200_OK)
...
def star_challenge(request, challenge_pk):
    ...
    if request.method == "POST":
        try:
            starred_challenge = StarChallenge.objects.get(user=request.user.pk, challenge=challenge)
...
def get_aws_credentials_for_participant_team(request, phase_pk):
    ...
    participant_team = get_participant_team_of_user_for_a_challenge(request.user, challenge.pk)
    if not challenge.is_docker_based:
...
def invite_users_to_challenge(request, challenge_pk):
    ...
    if not is_user_a_host_of_challenge(request.user, challenge.pk):
        response_data = {"error": "You're not authorized to invite a user in {}".format(challenge.title)}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
    ...
    for email in users_email:
        try:
            invited_user = UserInvitation.objects.get(email=email, challenge=challenge.pk)
        ...
        sender_email = settings.OPENCV_TEAM_EMAIL
        ...
        template_id = settings.SENDGRID_SETTINGS.get("TEMPLATES").get("CHALLENGE_INVITATION")
...
def accept_challenge_invitation(request, invitation_key):
    try:
        ...
        response_data = {"error": "The invitation with key {} doesn't exist".format(invitation_key)}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
    ...
    if request.method == "GET":
        ...
        serializer = UserDetailsSerializer(invitation.user, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            data = {"password": make_password(serializer.data.get("password"))}
            serializer = UserDetailsSerializer(invitation.user, data=data, partial=True)
            if serializer.is_valid():
                serializer.save()
            data = {"status": UserInvitation.ACCEPTED}
            serializer = UserInvitationSerializer(invitation, data=data, partial=True)
            if serializer.is_valid():
                serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def get_challenge_by_queue_name(request, queue_name):
    ...
    try:
        ...
        response_data = {"error": "Challenge with queue name {} does not exist".format(queue_name)}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
...
@permission_classes((permissions.IsAuthenticated, HasVerifiedEmail))
@authentication_classes((JWTAuthentication, ExpiringTokenAuthentication))
def get_challenge_by_title(request):
    """
    API endpoint to fetch the challenge details using the title attribute.
    Expects a query parameter 'title' with the challenge title.
    """
    ...
    try:
        ...
        response_data = {"error": f"Challenge with title '{title}' does not exist."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
...
def get_challenge_phases_by_challenge_pk(request, challenge_pk):
    ...
    serializer = ChallengePhaseCreateSerializer(challenge_phases, context={"request": request}, many=True)
    response_data = serializer.data
    return Response(response_data, status=status.HTTP_200_OK)
...
def get_challenge_phase_by_slug(request, slug):
    ...
    try:
        ...
        response_data = {"error": "Challenge phase with slug {} does not exist".format(slug)}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
    serializer = ChallengePhaseSerializer(challenge_phase)
    response_data = serializer.data
    return Response(response_data, status=status.HTTP_200_OK)
...
def get_challenge_phase_environment_url(request, slug):
    ...
    if not is_user_a_host_of_challenge(request.user, challenge.pk):
        response_data = {"error": "Sorry, you are not authorized to access test environment URL."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
    if not challenge.is_docker_based:
        response_data = {"error": "The challenge doesn't require uploading Docker images, hence no test environment URL."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
...
def get_challenge_evaluation_cluster_details(request, challenge_pk):
    ...
    if not is_user_a_host_of_challenge(request.user, challenge.pk):
        response_data = {"error": "Sorry, you are not authorized to access evaluation cluster details."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)

    if not challenge.is_docker_based:
        response_data = {"error": "The challenge doesn't require uploading Docker images, hence no evaluation cluster details."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        response_data = {"error": "Challenge evaluation cluster for the challenge with pk {} does not exist".format(challenge.pk)}
        return Response(response_data, status=status.HTTP_404_NOT_FOUND)
    serializer = ChallengeEvaluationClusterSerializer(challenge_evaluation_cluster, context={"request": request})
    response_data = serializer.data
    return Response(response_data, status=status.HTTP_200_OK)
...
def validate_challenge_config(request, challenge_host_team_pk):
    ...
    challenge_queryset = Challenge.objects.filter(github_repository=request.data["GITHUB_REPOSITORY"])

    data = request.data
    challenge_config_serializer = ChallengeConfigSerializer(data=data, context={"request": request})
    ...
    is_success, error_description = download_and_write_file(uploaded_zip_file_path, True, CHALLENGE_ZIP_DOWNLOAD_LOCATION, "wb")
    ...
    error_messages, yaml_file_data, files = validate_challenge_config_util(
        request, challenge_host_team, BASE_LOCATION, unique_folder_name, zip_ref, challenge_queryset[0] if challenge_queryset else None
    )
...
def get_worker_logs(request, challenge_pk):
    if not is_user_a_host_of_challenge(request.user, challenge_pk):
        response_data = {"error": "Sorry, you are not authorized to access the worker logs."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
    ...
    logs = get_logs_from_cloudwatch(log_group_name, log_stream_prefix, start_time, end_time, pattern, limit)
...
def scale_resources_by_challenge_pk(request, challenge_pk):
    ...
    """
    if not is_user_a_host_of_challenge(request.user, challenge_pk):
        response_data = {"error": "Sorry, you are not authorized for access worker operations."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
    ...
    if request.data.get("worker_memory") is None:
        response_data = {"error": "Worker memory config missing from request data."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
    ...
    if (
        ...
        if response["ResponseMetadata"]["HTTPStatusCode"] != 200:
            if response.get("Error", {"Message": "No error", "Code": "No error"}).get("Code", "No error code") == "ClientException":
                response_data = {"error": "Challenge workers are inactive or do not exist."}
            else:
                response_data = {"error": "Issue with ECS."}
            return Response(response_data, status=status.HTTP_503_SERVICE_UNAVAILABLE)
        elif response.get("Message", "N/A") == "Worker not modified":
            response_data = {"Success": "The challenge's worker cores and memory were not modified."}
        else:
            response_data = {"Success": "Worker scaled successfully!"}
    else:
        response_data = {"error": "Please specify correct config for worker vCPU and memory."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
    return Response(response_data, status=status.HTTP_200_OK)
...
def manage_worker(request, challenge_pk, action):
    ...
    if action not in ("start", "stop", "restart", "delete"):
        response_data = {"error": "The action {} is invalid for worker".format(action)}
        return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
    ...
    if challenge.end_date < pytz.UTC.localize(datetime.utcnow()) and action in ("start", "stop", "restart"):
        response_data = {"error": "Action {} worker is not supported for an inactive challenge.".format(action)}
        return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
...
def get_ec2_instance_details(request, challenge_pk):
    if not request.user.is_staff:
        response_data = {"error": "Sorry, you are not authorized for access EC2 operations."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)

    challenge = get_challenge_model(challenge_pk)

    if not challenge.uses_ec2_worker:
        response_data = {"error": "Challenge does not use EC2 worker instance."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
...
def delete_ec2_instance_by_challenge_pk(request, challenge_pk):
    ...
    if not challenge.ec2_instance_id:
        response_data = {"error": "No EC2 instance ID found for the challenge. Please ensure instance ID exists."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
...
def create_ec2_instance_by_challenge_pk(request, challenge_pk):
    ...
    if challenge.end_date < pytz.UTC.localize(datetime.utcnow()):
        response_data = {"error": "Creation of EC2 instance is not supported for an inactive challenge."}
        return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)

    if ec2_storage and not isinstance(ec2_storage, int):
        response_data = {"error": "Passed value of EC2 storage should be integer."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)

    if worker_instance_type and not isinstance(worker_instance_type, str):
        response_data = {"error": "Passed value of worker instance type should be string."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)

    if worker_image_url and not isinstance(worker_image_url, str):
        response_data = {"error": "Passed value of worker image URL should be string."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
    ...
    response = create_ec2_instance(challenge, ec2_storage, worker_instance_type, worker_image_url)
...
def manage_ec2_instance(request, challenge_pk, action):
    ...
    if challenge.end_date < pytz.UTC.localize(datetime.utcnow()) and action in ("start", "restart"):
        response_data = {"error": "Action {} EC2 instance is not supported for an inactive challenge.".format(action)}
        return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
...
def get_annotation_file_presigned_url(request, challenge_phase_pk):
    ...
    if not is_user_a_host_of_challenge(request.user, challenge_phase.challenge.pk):
        response_data = {"error": "Sorry, you are not authorized for uploading an annotation file."}
        return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
    ...
    if challenge_phase.test_annotation:
        file_key_on_s3 = "{}/{}".format(settings.MEDIAFILES_LOCATION, challenge_phase.test_annotation.name)
    else:
    ...
    response = generate_presigned_url_for_multipart_upload(file_key_on_s3, challenge_phase.challenge.pk, num_file_chunks)
...
def finish_annotation_file_upload(request, challenge_phase_pk):
    ...
    file_key_on_s3 = "{}/{}".format(settings.MEDIAFILES_LOCATION, challenge_phase.test_annotation.name)
    annotations_uploaded_using_cli = request.data.get("annotations_uploaded_using_cli")
    response = {}
    try:
        data = complete_s3_multipart_file_upload(file_parts, upload_id, file_key_on_s3, challenge_phase.challenge.pk)
        if data.get("error"):
            response_data = data
...
def create_or_update_github_challenge(request, challenge_host_team_pk):
    ...
    if challenge_queryset:
        ...
        if not is_user_a_host_of_challenge(request.user, challenge.pk):
            response_data = {"error": "Sorry, you are not a host for this challenge. Please check your user access token"}
            return Response(response_data, status=status.HTTP_403_FORBIDDEN)
    ...
    if not len(error_messages):
        if not challenge_queryset:
            ...
            try:
                ...
                response_data = {"error": f"Error in creating challenge: {error_messages}. Please check the yaml configuration!"}
                if error_messages:
            ...
            try:
                ...
                challenge_configuration = ChallengeConfiguration.objects.filter(challenge=challenge.pk).first()
                ...
                for data, challenge_test_annotation_file in zip(challenge_phases_data, files["challenge_test_annotation_files"]):
                    ...
                    challenge_phase = ChallengePhase.objects.filter(challenge__pk=challenge.pk, config_id=data["id"]).first()
                    if challenge_test_annotation_file and not challenge_phase.annotations_uploaded_using_cli:
                        serializer = ChallengePhaseCreateSerializer(
                            challenge_phase,
                            data=data,
                            context={
                    ...
                    elif challenge_test_annotation_file and challenge_phase.annotations_uploaded_using_cli:
                ...
                response_data = {"Success": "The challenge {} has been updated successfully".format(challenge.title)}
                return Response(response_data, status=status.HTTP_200_OK)
            except:  # noqa: E722
                response_data = {"error": f"Error in creating challenge: {error_messages}. Please check the yaml configuration!"}
                if error_messages:
...
def pwc_task_dataset(request):
    if request.user.is_staff:
        challenge_mapping = PWCChallengeLeaderboard.objects.filter(enable_sync=True)
        serializer = PWCChallengeLeaderboardSerializer(challenge_mapping, many=True, context={"request": request})
        response_data = serializer.data
        return Response(response_data, status=status.HTTP_200_OK)
    else:
        response_data = {"error": "You are not authorized to make this request. Please ask EvalAI admin to add you as a staff user for accessing this API."}
        return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
...
def update_allowed_email_ids(request, challenge_pk, phase_pk):
    ...
    try:
        challenge_phase = ChallengePhase.objects.get(challenge=challenge, pk=phase_pk)
    except ChallengePhase.DoesNotExist:
        response_data = {"error": "Challenge phase {} does not exist for challenge {}".format(phase_pk, challenge.pk)}
        return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
    ...
    if request.method != "GET":
        if request.data.get("allowed_email_ids") is not None:
            if not isinstance(request.data["allowed_email_ids"], list):
                response_data = {"error": "Field allowed_email_ids should be a list."}
                return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
        else:
    ...
    if request.method == "GET":
        ...
        allowed_email_ids = list(set(allowed_email_ids).difference(set(remove_allowed_email_ids)))
...
def request_challenge_approval_by_pk(request, challenge_pk):
    ...
    for challenge_phase in challenge_phases:
        submissions = Submission.objects.filter(challenge_phase=challenge_phase, status="finished")
...
def update_challenge_attributes(request):
    ...
    try:
        ...
        response_data = {"error": f"Challenge with primary key {challenge_pk} not found!"}
        return Response(response_data, status=status.HTTP_404_NOT_FOUND)
    ...
    response_data = {"message": f"Challenge attributes updated successfully for challenge with primary key {challenge_pk}!"}
    return Response(response_data, status=status.HTTP_200_OK)
    ...
    if request.method == "PUT":
        ...
        try:
            if leaderboard_data_pk:
                ...
                challenge_phase_split = get_challenge_phase_split_model(challenge_phase_split_pk)
                leaderboard = get_leaderboard_model(leaderboard_pk)
                leaderboard_data = LeaderboardData.objects.get(submission=submission, challenge_phase_split=challenge_phase_split, leaderboard=leaderboard)
        except Exception:
